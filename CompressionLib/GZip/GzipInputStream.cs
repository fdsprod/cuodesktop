...
namespace ICSharpCode.SharpZipLib.GZip
{
	...
	public class GZipInputStream : InflaterInputStream
	{
		...
		void ReadFooter()
		{
			...
			uint crcval = ( (uint)footer[0] & 0xff ) | ( ( (uint)footer[1] & 0xff ) << 8 ) | ( ( (uint)footer[2] & 0xff ) << 16 ) | ( (uint)footer[3] << 24 );
			if( crcval != (uint)crc.Value )
			{
				throw new GZipException("GZIP crc sum mismatch, theirs \"" + crcval + "\" and ours \"" + (uint)crc.Value);
			}
